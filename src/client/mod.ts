...
function getWebCryptoHashAlgorithm(algorithm: HashAlgorithm): string {
  switch (algorithm) {
    case "md5":
      throw new Error(
        "MD5 is not supported in browser (use SHA-256 or SHA-512)",
      );
    case "sha1":
      return "SHA-1";
    case "sha256":
...
export async function signJWT(
  ...
  if (algorithm.startsWith("HS")) {
    ...
    // RSA 或 ECDSA 签名
    if (!(secret instanceof CryptoKey)) {
      throw new Error("RSA/ECDSA signing requires a CryptoKey");
    }
...
export function hashPassword(
  ...
  return Promise.reject(
    new Error(`Password hash (${algorithm}) requires external library`),
  );
}
...
export function verifyPassword(
  ...
  // 注意：这里需要依赖外部库来实现 bcrypt 和 argon2
  return Promise.reject(
    new Error("Password verify requires external library"),
  );
}
